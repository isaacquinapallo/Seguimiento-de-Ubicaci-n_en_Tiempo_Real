...
class MainActivity : AppCompatActivity() {
    ...
    private val mensajeRunnable = object : Runnable {
        override fun run() {
            ...
            // No actualices aquí el texto porque se hace tras respuesta exitosa
    ...
    private fun enviarUbicacionASupabase(lat: Double, lon: Double) {
        ...
        val request = Request.Builder()
            ...
            .addHeader("Prefer", "return=representation")  // <-- Importante para aceptar el insert
            .build()

        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread {
                    tvMensaje.text = "Error al enviar ubicación: ${e.message}"
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                runOnUiThread {
                    if (response.isSuccessful) {
                        ...
                        tvMensaje.text = "Error en respuesta Supabase (${response.code}): $responseBody"
                    }
                }
            }
        })
